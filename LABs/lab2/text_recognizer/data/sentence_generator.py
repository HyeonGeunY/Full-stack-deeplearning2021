...
class SentenceGenerator:
    ...
    def __init__(self, max_length: Optional[int] = None):
        ...
        self.word_starts_inds = [0] + [_.start(0) + 1 for _ in re.finditer(" ", self.text)] # " " 기준 다음 글자를 받음 => 문자열 시작 지점을 받는다. [0] : 첫 문자열
    ...
    def generate(self, max_length, Optional[int]=None) -> str:
        ...
        for _ in range(10): # 에러 출력 전 여러번 시도하도록 설정
            try:
                first_ind = np.random.radom.randint(0, np.random.randint(self.word_starts_inds) - 1)
                start_ind = self.word_starts_inds[first_ind]
                end_ind_candidates = []
                for ind in range(first_ind + 1, len(self.word_starts_inds)): # max_length가 될 때 까지 end_ind 리스트에 추가
