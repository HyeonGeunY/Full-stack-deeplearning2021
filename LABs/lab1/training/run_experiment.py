...
def _import_class(module_and_class_name: str) -> type:
    ...
    module_name, class_name = module_and_class_name.rsplit(".", 1) # rsplit : "." 기준으로 오른쪽부터 두번째 인자 숫자 만큼 나눔 if 0 -> 나누지 않음.
...
def _setup_parser():
    ...
    trainer_parser._action_groups[1].title = "Trainer Args" # optional argument의 titel을 Trainer Args로 바꿈 [0] : postioner, [1]: optioner, [2]: pl.Trainer (pl에서 생성한 flags)
    parser = argparse.ArgumentParser(add_help=False, parents=[trainer_parser]) # pl.Trainer argparser의 모든 flag를 가지고 있는 parser 생성
...
def main():
    ...
    model = model_class(data_class=data.config(), args=args)
    ...
    if args.load_checkpoint is not None: # 기존 model를 load해서 사용할 떄 
        ...
        lit_models = lit_model_class(args=args, model=model)
