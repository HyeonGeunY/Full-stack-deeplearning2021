...
class EMNIST(BaseDataModule):
    ...
    def __init__(self, args=None):
        ...
        # 문자와 input 사이즈가 담겨있는 essential.json파일을 열어 
        # essential 변수에 저장. type: dict
        with open(ESSENTIALS_FILENAME) as f:
            essentials = json.load(f)

        self.mapping = list(essentials["characters"]) # mapping: 인덱스 => 문자
        self.inverse_mapping = {v: k for k, v in enumerate(self.mapping)} # inverse_mapping: chr => class
        self.transform = transforms.Compose([transforms.ToTensor()]) # 전처리 구성
        self.dims = (1, *essentials["input_shape"]) # input dimension (1, 28, 28) (채널, height, width)
        self.output_dims = (1,)
    ...
    def setup(self, stage: str = None) -> None:
        ...
        _process_raw_daaset()에서 처리한 데이터를 로드하여 사용한다.
        """
        if stage == "fit" or stage is None:
            # _process_raw_daaset()에서 처리한 데이터를 불러온다.
            with h5py.File(PROCESSED_DATA_FILENAME, "r") as f:
                self.x_trainval = f["x_train"][:]
                self.y_trainval = (
                    f["y_train"][:].squeeze().astype(int)
                )  # .squeeze() 1 로 구성된 dimension 제거, 자료형 int형으로 변환 

            # Dataset을 이용하여 Datamodule 내 self.data_train ~ 등을 할당한다.
...
def _sample_to_balance(x, y):
    ...
    for label in np.unique(y.flatten()):  # np.unique(np.array) 입력 array에 담긴 고유한 값들만 추출한 array반환
