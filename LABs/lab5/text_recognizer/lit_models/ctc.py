...
def compute_input_lengths(padded_sequence: torch.Tensor) -> torch.Tensor:
    """
    Parameters
    -------------
    padded_sequence : (N, S)
        0 은 padding에 해당하는 텐서


    Returns
    --------
    torch.Tensor : (N, )
        각 요소가 non-padded length에 해당하는 텐서
...
class CTCLitModel(BaseLitModel):
    ...
    def __init__(self, model, args: argparse.Namespace = None):
        ...
        self.loss_fn = torch.nn.CTCLoss(zero_infinity=True)  # (?) zero_infinity??
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logprobs_for_loss = logprobs.permute(2, 0, 1)  # -> (S, B, C) # C 와 y 값을 매칭 y : (B, S)
    ...
    def greedy_decode(self, logprobs: torch.Tensor, max_length: int) -> torch.Tensor:
        """
        sequence를 greedy 하게 decode, 반복되는 토큰을 합치고, CTC blank 토큰을 제거한다.
