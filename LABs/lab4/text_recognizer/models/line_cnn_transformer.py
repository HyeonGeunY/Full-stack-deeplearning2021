...
class LineCNNTransformer(nn.Module):
    ...
    def __init__(self, data_config: Dict[str, Any], args: argparse.Namespace = None) -> None:
        ...
        # Instantiate Line CNN with "num_classes" set to self.dim
    ...
    def decode(self, x, y):
        ...
        y = self.embedding(y.to(dtype=torch.long)) * math.sqrt(self.dim) # (Sy, B, E)
        y = self.pos_encoder(y) 
        Sy = y.shape[0]
        y_mask = self.y_mask[:Sy, :Sy].type_as(x)
        output = self.transformer_decoder(tgt=y, memory=x, tgt_mask=y_mask, tgt_key_padding_mask=y_padding_mask)
